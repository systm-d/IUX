...
final class IuxValueTokens {
  ...
  /// Space between the reading and the capsule's edge.
  ///
  /// Tighter than a chip's. A chip is a control with a target under it; this is
  /// four characters saying how far a measurement fell from its reference, and
  /// it sits above a word and below another number in a column that has to fit
  /// four rows on a phone.
  final EdgeInsets padding;

  /// Space between the capsule and the word under it.
  final double gap;

  /// The reading's style, already carrying [foreground].
  final TextStyle textStyle;

  /// The word's style, already carrying [foreground].
  ///
  /// Smaller than the reading and painted on the page rather than on the
  /// capsule, so the theme holds the same colour to 4.5:1 twice.
...
abstract final class IuxValueResolver {
  /// Resolves the tokens for [value] at [context].
  ///
  /// Reads the whole reading rather than its direction, and that is the change
  /// `ADR-0015` records: which side of a reference a number fell on does not
  /// determine which hue it is drawn in. [IuxValue.accent] does, and it is the
  /// caller's — except for a reading level with its reference, which has no
  /// accent to give and resolves the resting role.
  static IuxValueTokens resolve(BuildContext context, IuxValue value) {
    final IuxGeometryTheme geometry = IuxGeometryTheme.of(context);
    final IuxTypographyTheme typography = IuxTypographyTheme.of(context);
    final IuxSemanticColors colors = IuxSemanticColors.of(context);
    ...
    final IuxComparisonRoleColors role = switch (value.accent) {
      null => colors.comparison.neutral,
      IuxValueAccent.one => colors.comparison.one,
      IuxValueAccent.two => colors.comparison.two,
      IuxValueAccent.three => colors.comparison.three,
      IuxValueAccent.four => colors.comparison.four,
    };
    ...
    return IuxValueTokens(
      ...
      textStyle: typography.label.copyWith(color: role.content),
      // The word is the interpretation, not the measurement, so it is drawn
      // one step down the ramp — and it is the same colour, because a word in
      // a different hue from the capsule it explains is two signals for one
      // fact.
      meaningStyle: typography.supporting.copyWith(color: role.content),
    );
  }
}
